  ...
  try {
    ...
    if (data.success) {
      if (data.loggedIn) {
        ...
        console.log(
          "   node check-humanizer-auth.js login <email> <password>\n"
        );
...
async function login(email, password) {
  ...
  try {
    ...
    if (data.success) {
      ...
      console.log(
        "   - Try with headless mode disabled: HUMANIZER_HEADLESS=false\n"
      );
      return false;
    }
  } catch (error) {
...
async function main() {
  ...
  if (args[0] === "login") {
    ...
    if (!email || !password) {
      ...
      console.log(
        "Usage: node check-humanizer-auth.js login <email> <password>"
      );
